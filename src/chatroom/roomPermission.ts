...
async function getRoomPermission(loginToken: string): Promise<RoomPermission | null> {
  ...
  try {
    ...
    for (let i = 0; i < response.data.holdings.length; i++) {
      ...
      const balanceEthValue = response.data.holdings[i].balanceEthValue;
      const chatRoomId = response.data.holdings[i].chatRoomId;
      const twitterNameNoEmoji = twitterName
        .replace(/([\uE000-\uF8FF]|\uD83C[\uDF00-\uDFFF]|\uD83D[\uDC00-\uDDFF])/g, '')
        .trim();
      // console.log(
      //   `ROOM PERMISSION: \n username: ${username} | twitter name: ${twitterNameNoEmoji} | holding: ${balanceHolding} | eth value: ${balanceEthValue}`,
      // );
      chatroomHolding.push({
        username,
        twitterNameNoEmoji,
...
async function manageChannelsPermission(
  ...
  try {
    ...
    // console.log('value: ', existingChannels.values());
    const requiredChannelsNames = data.holdings.map(
      (room) => chatRoomPrefix + room.name.replace(/[^a-zA-Z0-9]/g, '-').toLowerCase(),
    );
    console.log('@REQUIRECHANNELNAME: ', requiredChannelsNames);
    // delete channels
    for (const channel of existingChannels.values()) {
      if (!requiredChannelsNames.includes(channel.name)) {
        await channel.delete();
      }
    }
    //create channel if not exists
